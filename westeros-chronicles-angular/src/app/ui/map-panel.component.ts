...
export class MapPanelComponent {
  ...
  private fastTravelCost(fromLocationId: string, toLocationId: string): number {
    if (fromLocationId === toLocationId) return 0;
    const from = this.state.locations[fromLocationId];
    const to = this.state.locations[toLocationId];
    if (!from || !to) return 18;
    const sameRegion = from.regionId === to.regionId;
    return sameRegion ? 8 : 18;
  }
  ...
  areasInSelected(): Array<{label: string, id: string, hint: string, disabled: boolean}> {
    ...
    return locs.map(l => {
      ...
      const cost = this.fastTravelCost(here.id, l.id);
      if (l.id === here.id) {
        return {
          label: l.name,
          id: `go:${l.id}`,
          hint: 'Você já está aqui',
          disabled: true,
        };
      }
      return {
        ...
        id: `go:${l.id}`,
        hint: reachable
          ? `Rota direta (sem custo de ouro)`
          : `Viagem longa (sem rota direta) • custa ${cost} ouro pessoal`,
        disabled: false,
      };
    });
  }
  ...
  travelOptions(): Array<{label: string, id: string, hint: string}> {
    ...
    return edges.map(e => {
      ...
      return {
        label: to.name,
        id: `go:${to.id}`,
        hint: `Distância ${e.distance} • rota direta (sem custo de ouro)`,
      };
    });
  }
